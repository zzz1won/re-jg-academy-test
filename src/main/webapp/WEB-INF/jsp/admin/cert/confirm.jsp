                	...
                	<form id="searchForm" name="searchForm">
	                    <ul class="filter-row">
	                        ...
	                        <li>
	                            ...
	                            <input type="text" id="eduTitle" name="eduTitle" class="input-text" style="width:350px" value="<c:out value="${search.eduTitle}"/>"/>
	                        </li>
	                        <li>
	                        ...
	                        <li>
	                            ...
	                            <input type="text" id="judgeNo" name="judgeNo" class="input-text" style="width:140px" value="<c:out value="${search.judgeNo}"/>"/>
	                        </li>
	                        <li>
                    ...
                    <table id="listTable" class="cell-border hover dataTable" width="100%">
                        ...
                        <tbody>
                            <c:forEach var="cert" items="${certList}" varStatus="status">
								<tr>
									<td>
										<input type="checkbox" id="chk${cert.eduApplyInfoNo}" name="chk" value="<c:out value="${cert.acEduCertInfoNo}"/>">
									</td>
									...
									<td>
										...
										<c:when test="${cert.state eq '03'}">
											...
											<c:when test="${not empty cert.acEduCertFilePath}">
												<a href="javascript:fn_view_cert('<c:out value="${cert.acEduCertInfoNo}"/>');" class="btn-num">등록완료</a>
											</c:when>
											<%-- 수료증 미등록되어있는 경우 --%>
											<c:otherwise>
												<a href="javascript:fn_register_cert('<c:out value="${cert.acEduCertInfoNo}"/>', '<c:out value="${cert.eduApplyInfoNo}"/>', '<c:out value="${cert.judgeNo}"/>');" class="btn-num">등록전</a>
											</c:otherwise>
											</c:choose>
										</c:when>
										<%-- 수료상태 아님 --%>
